...
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
@property (nonatomic, strong) id <UPITurboStandardUIPlugin> _Nullable upiTurbo;
...
+ (RazorpayCheckout * _Nonnull)initWithKey:(NSString * _Nonnull)key andDelegateWithData:(id <RazorpayPaymentCompletionProtocolWithData> _Nonnull)delegate plugin:(id <UPITurboStandardUIPlugin> _Nullable)plugin SWIFT_METHOD_FAMILY(none) SWIFT_WARN_UNUSED_RESULT;
...
SWIFT_PROTOCOL("_TtP8Razorpay24UPITurboStandardUIPlugin_")
@protocol UPITurboStandardUIPlugin <UPITurboUIPlugin>
- (void)makePaymentWithPayload:(NSDictionary * _Nonnull)payload upiAccount:(id _Nullable)upiAccount delegate:(id <RazorpayPaymentCompletionProtocolWithData> _Nullable)delegate;
@end
